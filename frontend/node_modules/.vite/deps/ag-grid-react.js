...
import { t as require_react_dom } from "./react-dom-CVUXoGTx.js";
import { At as GridCtrl, Ci as _isCellSelectionEnabled, Fi as _isHeaderFocusSuppressed, Ho as AgPromise, Li as _isMultiRowSelection, N as BaseComponentWrapper, Nt as HeaderRowContainerCtrl, Ot as GridBodyCtrl, Pn as VanillaFrameworkOverrides, R as BeanStub, Ta as applyHeaderWrapperMaxHeight, Tn as TabGuardCtrl, Ui as _isServerSideRowModel, Ur as _getGridOption, Wr as _getGridRegisteredModules, Zn as _combineAttributesAndGridOptions, _t as FakeVScrollComp, ai as _getRowContainerOptions, bo as partitionByPinned, fi as _getRowSpanContainerClass, gt as FakeHScrollComp, ii as _getRowContainerClass, jt as GridHeaderCtrl, kt as GridCoreCreator, mr as _findEnterpriseCoreModule, on as RowContainerCtrl, pt as EmptyBean, ra as _processOnChange, wa as applyHeaderWrapperHidden, wi as _isClientSideRowModel, xa as _warn, zo as updatePinnedSectionWidths } from "./main.esm-CDA3vZCz.js";
//#region node_modules/ag-grid-react/dist/package/index.esm.mjs
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var import_react_dom = /* @__PURE__ */ __toESM(require_react_dom(), 1);
var VERSION = "36.0.2";
typeof window !== "object" || window?.document?.fonts?.forEach;
...
var LocalEventService = class {
	...
	dispatchAsync(func) {
		...
		if (!this.scheduled) {
			...
			if (frameworkOverrides) frameworkOverrides.wrapIncoming(flush);
			else flush();
			this.scheduled = true;
		}
	}
	flushAsyncQueue() {
		...
		this.asyncFunctionsQueue = [];
		for (const func of queueCopy) func();
	}
};
function _areEqual(a, b, comparator) {
	...
	if (comparator) {
		for (let i = 0; i < len; ++i) {
			const valueA = a[i];
			const valueB = b[i];
			if (valueA !== valueB && !comparator(valueA, valueB)) return false;
		}
		return true;
	}
...
function _getDocument(beans) {
	...
	if (optionsGetDocument && _exists(optionsGetDocument)) result = optionsGetDocument();
	else if (eRootDiv) result = eRootDiv.ownerDocument;
...
var AgBeanStub = class {
	...
	_setupListener(object, event, listener) {
		...
		if (isAgEventEmitter(object)) {
			...
			if (object instanceof HTMLElement) _addSafePassiveEventListener(object, event, listener);
			else if (objIsEventService) object.addListener(event, listener);
	...
	*/
	addManagedPropertyListeners(events, listener) {
		...
		const wrappedListener = (event) => {
			...
			if (changeSet) {
				...
				if (changeSet.id === lookup[eventsKey]) return;
		...
		for (let i = 0, len = events.length; i < len; ++i) this.setupPropertyListener(events[i], wrappedListener);
	}
	isAlive() {
	...
	pushDestroyFunc(destroyFunc) {
		const destroyFunctions = this.destroyFunctions;
		if (destroyFunctions) destroyFunctions.push(destroyFunc);
		else this.destroyFunctions = [destroyFunc];
	}
	...
	registerDestroyFunc(destroyFunc) {
		...
		return () => {
			destroyFunc();
			const destroyFunctions = this.destroyFunctions;
			if (destroyFunctions) _removeFromArray(destroyFunctions, destroyFunc);
			return null;
		};
	}
	addDestroyFunc(func) {
		if (this.destroyed) func();
		else this.pushDestroyFunc(func);
	}
...
var CssClassManager = class {
	...
	toggleCss(className, addOrRemove) {
		...
		if (className.includes(" ")) {
			...
			if (list.length > 1) {
				for (const cls of list) this.toggleCss(cls, addOrRemove);
				return;
			}
		}
...
new RegExp(`[${DATE_TIME_SEPARATOR} ]`);
new RegExp(`^\\d{4}-\\d{2}-\\d{2}(${DATE_TIME_SEPARATOR}\\d{2}:\\d{2}:\\d{2}\\D?)?`);
...
var showJsComp = (compDetails, context, eParent, ref) => {
	...
	promise.then((c) => {
		...
		comp = c;
		compGui = comp.getGui?.();
		if (compGui) eParent.appendChild(compGui);
		setRef(ref, comp);
	});
	return () => {
		destroyed = true;
...
var reactVersion = "19.2.8".split(".")[0];
...
var GroupCellRenderer = (0, import_react.forwardRef)((props, ref) => {
	...
	const [expandedAriaHidden, setExpandedAriaHidden] = (0, import_react.useState)(true);
	const [contractedCssClasses, setContractedCssClasses] = (0, import_react.useState)(() => new CssClasses("ag-hidden"));
	const [contractedAriaHidden, setContractedAriaHidden] = (0, import_react.useState)(true);
	const [checkboxCssClasses, setCheckboxCssClasses] = (0, import_react.useState)(() => new CssClasses("ag-invisible"));
	const [checkboxAriaHidden, setCheckboxAriaHidden] = (0, import_react.useState)(true);
...
var ModulesContext = import_react.createContext(null);
...
var HeaderGroupCellComp = ({ ctrl }) => {
	...
	const setRef2 = (0, import_react.useCallback)((eRef) => {
		...
		ctrl.setComp({
			...
			setHeaderWrapperHidden: (hidden) => {
				if (eHeaderCompWrapper.current) applyHeaderWrapperHidden(eHeaderCompWrapper.current, hidden);
			},
			setHeaderWrapperMaxHeight: (value) => {
				if (eHeaderCompWrapper.current) applyHeaderWrapperMaxHeight(eHeaderCompWrapper.current, value);
			},
			setUserCompDetails: (compDetails) => setUserCompDetails(compDetails),
	...
	(0, import_react.useLayoutEffect)(() => showJsComp(userCompDetails, context, eHeaderCompWrapper.current, userCompRef), [context, userCompDetails]);
	(0, import_react.useEffect)(() => {
...
var HeaderRowComp = ({ ctrl, setGuiRef }) => {
	...
	const eGui = (0, import_react.useRef)(null);
	const ePinnedLeft = (0, import_react.useRef)(null);
	const eScrolling = (0, import_react.useRef)(null);
	const ePinnedRight = (0, import_react.useRef)(null);
	const compBean = (0, import_react.useRef)();
	const cellCtrlsRef = (0, import_react.useRef)([]);
	const prevCellCtrlsRef = (0, import_react.useRef)([]);
	const sectionSignatureRef = (0, import_react.useRef)("");
	const domOrderRef = (0, import_react.useRef)(false);
	const [cellCtrls, setCellCtrls] = (0, import_react.useState)([]);
	const pinnedWidthsCache = (0, import_react.useRef)({
		pinnedLeftWidth: void 0,
		centerWidth: void 0,
		pinnedRightWidth: void 0
	});
	const refreshPinnedWidths = (0, import_react.useCallback)(() => {
		if (!ePinnedLeft.current || !eScrolling.current || !ePinnedRight.current) return;
		const isPrint2 = gos.get("domLayout") === "print";
		updatePinnedSectionWidths(visibleCols, isPrint2, {
	...
	const setRef2 = (0, import_react.useCallback)((eRef) => {
		...
		const updateCellCtrls = (useFlushSync) => {
			...
			const next = sectionSignatureRef.current !== nextSectionSignature ? cellCtrlsRef.current : getNextValueIfDifferent(prevCellCtrlsRef.current, cellCtrlsRef.current, domOrderRef.current);
		...
		ctrl.setComp({
			setTop: (value) => {
				if (eGui.current) eGui.current.style.top = value;
			},
			setHeight: (value) => {
				if (eGui.current) eGui.current.style.height = value;
			},
			...
			setWidth: (value) => {
				if (eGui.current) eGui.current.style.width = value;
			},
	...
	const { left: leftCells, center: centerCells, right: rightCells } = (0, import_react.useMemo)(() => {
		if (isPrint) return {
			left: [],
			center: cellCtrls,
			right: []
		};
		return partitionByPinned(cellCtrls, (ctrl2) => ctrl2.column.getPinned());
	}, [cellCtrls, isPrint]);
	...
	const tabIndex = _isHeaderFocusSuppressed(beans) ? void 0 : gos.get("tabIndex");
	return /* @__PURE__ */ import_react.createElement("div", {
		ref: setRef2,
		...
		role: "presentation"
	}, leftCells.map(createCellJsx))), /* @__PURE__ */ import_react.createElement("div", {
		ref: eScrolling,
		className: "ag-grid-scrolling-cells",
		role: "presentation"
	}, centerCells.map(createCellJsx)), /* @__PURE__ */ import_react.createElement("div", {
		ref: ePinnedRight,
...
var HeaderRowsComp = ({ eGui, eGridViewport, setHeaderRowFocusableElements }) => {
	...
	const headerRowContainerCtrlRef = (0, import_react.useRef)();
	const rowGuisRef = (0, import_react.useRef)(/* @__PURE__ */ new Map());
	const setRowGui = (0, import_react.useCallback)((instanceId, eGui2) => {
		if (eGui2) rowGuisRef.current.set(instanceId, eGui2);
		else rowGuisRef.current.delete(instanceId);
	}, []);
	(0, import_react.useLayoutEffect)(() => {
		if (!setHeaderRowFocusableElements) return;
		setHeaderRowFocusableElements(headerRowCtrls.map((ctrl) => rowGuisRef.current.get(ctrl.instanceId)).filter((eGui2) => !!eGui2));
	...
	(0, import_react.useLayoutEffect)(() => {
		...
		return () => {
			if (setHeaderRowFocusableElements) setHeaderRowFocusableElements([]);
			headerRowContainerCtrlRef.current = context.destroyBean(headerRowContainerCtrlRef.current);
		};
	}, [
		context,
	...
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, headerRowCtrls.map((ctrl) => /* @__PURE__ */ import_react.createElement(headerRowComp_default, {
		ctrl,
		key: ctrl.instanceId,
		setGuiRef: (eGui2) => setRowGui(ctrl.instanceId, eGui2)
	})));
};
var headerRowsComp_default = (0, import_react.memo)(HeaderRowsComp);
var GridHeaderComp = ({ eTopSection, eGridViewport }) => {
	const { context, environment } = (0, import_react.useContext)(BeansContext);
	const gridHeaderCtrlRef = (0, import_react.useRef)();
	const cssManager = (0, import_react.useRef)();
	const eGui = (0, import_react.useRef)(null);
	...
	const setHeaderRowFocusableElements = (0, import_react.useCallback)((elements) => {
	...
	const setRef2 = (0, import_react.useCallback)((eRef) => {
		...
		const compProxy = {
			...
			setHeightAndMinHeight: (height) => {
				const heightWithBorder = height + environment.getHeaderRowBorderWidth();
				eTopSection.style.setProperty("--ag-header-rows-height", `${heightWithBorder}px`);
				if (eGui.current) eGui.current.style.height = `${heightWithBorder}px`;
			}
		};
	...
	return /* @__PURE__ */ import_react.createElement("div", {
		ref: setRef2,
		role: "presentation"
	}, mounted && headerElement && /* @__PURE__ */ import_react.createElement(headerRowsComp_default, {
		eGui: headerElement,
		eGridViewport,
		setHeaderRowFocusableElements
	}));
...
var CellComp = ({ cellCtrl, printLayout, editingCell }) => {
	...
	(0, import_react.useLayoutEffect)(() => {
		...
		if (oldCompDetails == null || newCompDetails == null || oldCompDetails === newCompDetails) return;
...
var RowComp = ({ rowCtrl, containerType }) => {
	...
	const [cellCtrlsFlushSync, setCellCtrlsFlushSync] = (0, import_react.useState)(() => rowCtrl.getInitialCellCtrls(containerType));
	const cellCtrlsRef = (0, import_react.useRef)(cellCtrlsFlushSync);
	const [fullWidthCompDetails, setFullWidthCompDetails] = (0, import_react.useState)();
	const [embeddedFullWidthCompDetails, setEmbeddedFullWidthCompDetails] = (0, import_react.useState)();
	const embeddedFullWidthCompDetailsRef = (0, import_react.useRef)();
	const [top, setTop] = (0, import_react.useState)(() => isDisplayed ? rowCtrl.getInitialRowTop() : void 0);
	const [transform, setTransform] = (0, import_react.useState)(() => isDisplayed ? rowCtrl.getInitialTransform() : void 0);
	const eGui = (0, import_react.useRef)(null);
	const eFullWidthAnchor = (0, import_react.useRef)(null);
	const ePinnedLeftCells = (0, import_react.useRef)(null);
	const eScrollingCells = (0, import_react.useRef)(null);
	const ePinnedRightCells = (0, import_react.useRef)(null);
	const fullWidthCompRef = (0, import_react.useRef)();
	const fullWidthEmbeddedLeftCompRef = (0, import_react.useRef)();
	const fullWidthEmbeddedCenterCompRef = (0, import_react.useRef)();
	const fullWidthEmbeddedRightCompRef = (0, import_react.useRef)();
	const fullWidthParamsRef = (0, import_react.useRef)();
	const fullWidthEmbeddedLeftParamsRef = (0, import_react.useRef)();
	const fullWidthEmbeddedCenterParamsRef = (0, import_react.useRef)();
	const fullWidthEmbeddedRightParamsRef = (0, import_react.useRef)();
	const [, setEmbeddedSectionHasContent] = (0, import_react.useState)(() => rowCtrl.embeddedSectionHasContent);
	const [, refreshWidths] = (0, import_react.useState)(0);
	...
	const setRef2 = (0, import_react.useCallback)((eRef) => {
		...
		const compProxy = {
			...
			getFullWidthCellRenderers: () => {
				if (rowCtrl.isEmbeddedFullWidth) return [
			...
			refreshEmbeddedFullWidth: (getUpdatedParams) => {
				...
				const refreshSection = (section, params, renderer, hasContent) => {
					...
					if (!!details?.componentFromFramework && isComponentStateless(details.componentClass)) {
						if (!gos.get("reactiveCustomComponents") || !currentDetails) return false;
	...
	(0, import_react.useLayoutEffect)(() => showJsComp(fullWidthCompDetails, context, eFullWidthAnchor.current ?? eGui.current, fullWidthCompRef), [fullWidthCompDetails]);
	(0, import_react.useLayoutEffect)(() => {
		if (!ePinnedLeftCells.current) return;
	...
	(0, import_react.useLayoutEffect)(() => {
		if (!eScrollingCells.current) return;
	...
	(0, import_react.useLayoutEffect)(() => {
		if (!ePinnedRightCells.current) return;
	...
	(0, import_react.useLayoutEffect)(() => {
		if (!showEmbeddedFullWidth) return;
		const updateLaneVisibility = () => {
			...
			setEmbeddedSectionHasContent((prev) => prev.left === next.left && prev.center === next.center && prev.right === next.right ? prev : next);
		};
		updateLaneVisibility();
		const observer = new MutationObserver(updateLaneVisibility);
		if (ePinnedLeftCells.current) observer.observe(ePinnedLeftCells.current, { childList: true });
		if (eScrollingCells.current) observer.observe(eScrollingCells.current, { childList: true });
		if (ePinnedRightCells.current) observer.observe(ePinnedRightCells.current, { childList: true });
	...
	const { leftCellCtrls, centerCellCtrls, rightCellCtrls } = (0, import_react.useMemo)(() => {
		const left = [];
		const center = [];
		const right = [];
		for (const cellCtrl of cellCtrlsMerged ?? []) {
			const pinned = cellCtrl.column.getPinned();
			if (pinned === "left") left.push(cellCtrl);
			else if (pinned === "right") right.push(cellCtrl);
			else center.push(cellCtrl);
		}
		return {
			leftCellCtrls: left,
	...
	const showCellsJsx = (cellCtrls) => cellCtrls.map((cellCtrl) => /* @__PURE__ */ import_react.createElement(cellComp_default, {
		cellCtrl,
	...
	const showEmbeddedFrameworkSection = (section) => {
		...
		if (!details?.componentFromFramework) return null;
		...
		return isComponentStateless(details.componentClass) ? /* @__PURE__ */ import_react.createElement(FullWidthComp, { ...details.params }) : /* @__PURE__ */ import_react.createElement(FullWidthComp, {
			...details.params,
			ref: compRef
		});
	};
	const renderCellSection = (sectionClass, ref, width, children, pinned = false, shouldRender = true) => {
		...
		if (pinned) return /* @__PURE__ */ import_react.createElement("div", {
	...
	}, showCells || showEmbeddedFullWidth ? /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, renderCellSection("ag-grid-pinned-left-cells", ePinnedLeftCells, leftWidth, showCells ? showCellsJsx(leftCellCtrls) : showEmbeddedFrameworkSection("left"), true, renderLeft), renderCellSection("ag-grid-scrolling-cells", eScrollingCells, centerWidth, showCells ? showCellsJsx(centerCellCtrls) : showEmbeddedFrameworkSection("center")), renderCellSection("ag-grid-pinned-right-cells", ePinnedRightCells, rightWidth, showCells ? showCellsJsx(rightCellCtrls) : showEmbeddedFrameworkSection("right"), true, renderRight)) : showFullWidthFramework ? /* @__PURE__ */ import_react.createElement("div", {
		className: "ag-full-width-anchor",
		role: "presentation",
		ref: eFullWidthAnchor
	}, showFullWidthFrameworkJsx()) : isFullWidth ? /* @__PURE__ */ import_react.createElement("div", {
		className: "ag-full-width-anchor",
		role: "presentation",
		ref: eFullWidthAnchor
	}) : null);
...
var RowContainerComp = ({ name, viewportElement, extraClassName }) => {
	...
	const containerClasses = (0, import_react.useMemo)(() => classesList(_getRowContainerClass(name), hidden ? "ag-hidden" : null, extraClassName), [
		extraClassName,
		name,
		hidden
	]);
	...
	const setRef2 = (0, import_react.useCallback)(() => {
		if (eContainer.current == null && eSpanContainer.current == null) rowContainerCtrlRef.current = context.destroyBean(rowContainerCtrlRef.current);
		if (context.isDestroyed()) return;
		const eContainerForCtrl = eContainer.current;
		const eViewportForCtrl = viewportElement ?? eContainer.current;
		if (!eContainerForCtrl || !eViewportForCtrl || isSpanning && !eSpanContainer.current) return;
		if (rowContainerCtrlRef.current) return;
		...
		const compProxy = {
			...
			setContainerWidth: (width) => {
				if (eContainerForCtrl) eContainerForCtrl.style.width = width;
				if (eSpanContainerForCtrl) eSpanContainerForCtrl.style.width = width;
			},
			setOffsetTop: (offset) => {
				eContainerForCtrl.style.transform = `translateY(${offset})`;
				if (eSpanContainerForCtrl) eSpanContainerForCtrl.style.transform = `translateY(${offset})`;
			},
	...
	const rows = rowCtrlsOrdered.map((rowCtrl) => /* @__PURE__ */ import_react.createElement(rowComp_default, {
		rowCtrl,
		containerType: containerOptions.type,
		key: rowCtrl.instanceId
	}));
...
var GridBodyComp = () => {
	const { context, gos, overlays, rangeSvc } = (0, import_react.useContext)(BeansContext);
	const [rowAnimationClass, setRowAnimationClass] = (0, import_react.useState)("");
	const [pinnedSections, setPinnedSections] = (0, import_react.useState)({
		top: {
			height: 0,
			invisible: true
		},
	...
	const [preventRowAnimationClass, setPreventRowAnimationClass] = (0, import_react.useState)(null);
	...
	const [topElement, setTopElement] = (0, import_react.useState)(null);
	const eGridViewport = (0, import_react.useRef)(null);
	const [gridViewportElement, setGridViewportElement] = (0, import_react.useState)(null);
	const eGridScrollableArea = (0, import_react.useRef)(null);
	const eBody = (0, import_react.useRef)(null);
	const eBottom = (0, import_react.useRef)(null);
	const eTopExtraRows = (0, import_react.useRef)(null);
	...
	const setRootRef = (0, import_react.useCallback)((eRef) => {
		eRoot.current = eRef;
		setRootElement(eRef);
	}, []);
	const setPinnedSection = (0, import_react.useCallback)((section, state) => {
		setPinnedSections((prev) => {
			const current = prev[section];
			if (current.height === state.height && current.invisible === state.invisible) return prev;
			return {
				...prev,
				[section]: state
			};
	...
	(0, import_react.useEffect)(() => {
		if (!rootElement || context.isDestroyed() || !eGridViewport.current || !eBody.current || !eTop.current || !eBottom.current || !eTopExtraRows.current) return;
		...
		if (overlayComp) addComp(rootElement, overlayComp, " AG Overlay Wrapper ");
		const compProxy = {
			setColumnCount: (count) => {
				if (eGridViewport.current) _setAriaColCount(eGridViewport.current, count);
			},
			setRowCount: (count) => {
				if (eGridViewport.current) _setAriaRowCount(eGridViewport.current, count);
			},
			setPinnedSection,
			...
			setGridScrollableAreaWidth: (width) => {
				if (eGridScrollableArea.current) eGridScrollableArea.current.style.width = width;
			},
			setStickyBottomHeight,
			setStickyBottomWidth,
			setGridRole: (role) => {
				if (eGridViewport.current) _setAriaRole(eGridViewport.current, role);
			}
		};
		...
		if (eGridViewport.current && (rangeSvc && _isCellSelectionEnabled(gos) || _isMultiRowSelection(gos))) _setAriaMultiSelectable(eGridViewport.current, true);
		return () => {
			context.destroyBeans(beansToDestroy);
			for (const f of destroyFuncs) f();
		};
	}, [
		context,
		gos,
	...
	const gridViewportClasses = (0, import_react.useMemo)(() => classesList("ag-grid-viewport", layoutClass), [layoutClass]);
	const bodyClasses = (0, import_react.useMemo)(() => classesList("ag-grid-scrolling-rows", layoutClass, cellSelectableCss), [layoutClass, cellSelectableCss]);
	const topSection = pinnedSections.top;
	const bottomSection = pinnedSections.bottom;
	const topClasses = (0, import_react.useMemo)(() => classesList("ag-grid-pinned-top-rows", cellSelectableCss), [cellSelectableCss, topSection.invisible]);
	...
	const scrollableClasses = (0, import_react.useMemo)(() => classesList("ag-grid-scrollable-area", topSection.invisible ? null : "ag-has-top-pinned-rows", bottomSection.invisible ? null : "ag-has-bottom-pinned-rows"), [bottomSection.invisible, topSection.invisible]);
	const bottomClasses = (0, import_react.useMemo)(() => classesList("ag-grid-pinned-bottom-rows", bottomSectionHidden ? "ag-hidden" : null, cellSelectableCss), [
		bottomSection.invisible,
		bottomSectionHidden,
		cellSelectableCss
	]);
	const rowAnimationContainerClass = (0, import_react.useMemo)(() => classesList(rowAnimationClass, preventRowAnimationClass), [preventRowAnimationClass, rowAnimationClass]);
	...
	const setTopRef = (0, import_react.useCallback)((el) => {
		eTop.current = el;
		setTopElement(el);
	}, []);
	const setGridViewportRef = (0, import_react.useCallback)((el) => {
	...
	return /* @__PURE__ */ import_react.createElement("div", {
		...
		role: "presentation",
		style: topStyle
	}, topElement && gridViewportElement && /* @__PURE__ */ import_react.createElement(gridHeaderComp_default, {
		eTopSection: topElement,
		eGridViewport: gridViewportElement
	}), /* @__PURE__ */ import_react.createElement("div", {
		ref: eTopExtraRows,
		className: "ag-extra-rows-container",
		role: "presentation"
	}), /* @__PURE__ */ import_react.createElement(rowContainerComp_default, {
		name: "pinnedTop",
...
var GridComp = ({ context }) => {
	...
	(0, import_react.useEffect)(() => {
		...
		if (gridHeaderDropZonesSelector) {
			...
			if (toolbar) toolbar.after(eGui);
			else eRootWrapper.prepend(eGui);
			additionalEls.push(eGui);
	...
	const rootWrapperClasses = (0, import_react.useMemo)(() => classesList("ag-root-wrapper", layoutClass), [layoutClass]);
...
var AgGridReactUi = (props) => {
	...
	const innermostRef = (0, import_react.useRef)(null);
	...
	const setOutermostRef = (0, import_react.useCallback)((outermost) => {
		if (!outermost) {
			ready.current = false;
	...
	return /* @__PURE__ */ import_react.createElement("div", {
		...
		style,
		ref: setOutermostRef
	}, /* @__PURE__ */ import_react.createElement("div", null, /* @__PURE__ */ import_react.createElement("div", null, /* @__PURE__ */ import_react.createElement("div", { ref: innermostRef }, /* @__PURE__ */ import_react.createElement(RenderModeContext.Provider, { value: renderMode }, context && !context.isDestroyed() ? /* @__PURE__ */ import_react.createElement(gridComp_default, {
