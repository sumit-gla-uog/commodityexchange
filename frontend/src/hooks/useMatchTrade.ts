...
import type { FairValue, MatchedListing, SourceListing } from '../types/commodity'

type MatchStage = 'idle' | 'loading' | 'no-match' | 'found' | 'confirming' | 'submitting' | 'success' | 'error'
...
export const useMatchTrade = () => {
  ...
  const findMatch = async (listingId: string) => {
    setStage('loading')
    try {
      const res = await fetch(`${BASE_URL}/api/barter/match/${listingId}`, { method: 'POST' })
  ...
  const confirmTrade = async (onSuccess: () => void) => {
    ...
    try {
      const platformFee = Math.round(Math.abs(fairValue.delta_usd) * PLATFORM_FEE_RATE * 100) / 100
  ...
  const platformFee = fairValue ? Math.round(Math.abs(fairValue.delta_usd) * PLATFORM_FEE_RATE * 100) / 100 : 0
